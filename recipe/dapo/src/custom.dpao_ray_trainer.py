            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('reward', timing_raw):
                        ...
                        '''
                        if self.use_rm: # self.use_rm = Role.RewardModel in role_worker_mapping
                    ...
                    # @liugangao
                    '''
                    if not self.config.algorithm.filter_groups.enable: # algorithm.filter_groups.enable=True
                        batch = new_batch
                    else:  # NOTE: When prompts after filtering is less than train batch size, we skip to the next generation batch
                    '''
                    metric_name = self.config.algorithm.filter_groups.metric # acc
                    '''
                    ...
                    if self.use_reference_policy: # self.use_reference_policy = Role.RefPolicy in role_worker_mapping
